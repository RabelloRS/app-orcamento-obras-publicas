                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="w-1/2 h-12 bg-slate-100 rounded-lg relative overflow-hidden border border-slate-200">
                                <div id="cbr-bar" class="absolute top-0 left-0 h-full bg-gradient-to-r from-red-400 via-yellow-400 to-green-500 transition-all duration-500" 
                                     style="width: ${Math.min(state.subgradeCBR, 20) * 5}%"></div>
...
function attachListeners() {
    document.getElementById('input-cbr').addEventListener('input', (e) => {
        const val = Number(e.target.value);
        store.updateNested('soil.subgradeCBR', val);
        updateVisuals(val, store.getState().soil.expansion);
    });

    document.getElementById('input-exp').addEventListener('input', (e) => {
        const val = Number(e.target.value);
        store.updateNested('soil.expansion', val);
        updateVisuals(store.getState().soil.subgradeCBR, val);
    });
...
function updateVisuals(cbr, exp) {
    ...
    let width = Math.min(cbr, 20) * 5;
    bar.style.width = `${width}%`;

    let text = 'Fraco';
    if (cbr >= 5) text = 'Regular';
    if (cbr >= 10) text = 'Bom';
    if (cbr >= 20) text = 'Excelente';
    label.innerText = text;

    const expWarn = document.getElementById('exp-warning');
    if (exp > 2.0) expWarn.classList.remove('hidden');
    else expWarn.classList.add('hidden');
}
