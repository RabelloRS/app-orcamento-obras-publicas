...
export const render = (container) => {
    ...
    const getOptions = (selectedId, filterType = null) => {
        return MATERIALS.filter(m => !filterType || m.type === filterType || m.type === 'stabilized').map(m => `<option value="${m.id}" ${m.id === selectedId ? 'selected' : ''} data-k="${m.k}">${m.name} (K=${m.k})</option>`).join('');
    };
    container.innerHTML = `
        <div class="animate-fade-in flex flex-col lg:flex-row gap-6 h-full">
            <div class="w-full lg:w-1/2 flex flex-col gap-4 overflow-y-auto pb-8">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="card p-4 border-l-4 border-dnit-500"><span class="text-xs text-slate-500 uppercase">Espessura Total Necessária ($H_m$)</span><div class="text-2xl font-bold text-dnit-800">${H_total_needed.toFixed(1)} cm</div></div>
                    <div class="card p-4 border-l-4 border-emerald-500"><span class="text-xs text-slate-500 uppercase">Espessura Equivalente Atual</span><div class="text-2xl font-bold text-emerald-800" id="current-eq-thickness">0.0 cm</div></div>
                </div>
                <div class="space-y-4">
            ...
            <div class="w-full lg:w-1/2 card p-6 flex flex-col sticky top-0">
                ...
                <div class="mt-4 flex justify-between text-xs text-slate-400"><span>Subleito CBR ${subgradeCBR}%</span><span>N = ${state.traffic.calculatedN.toExponential(2)}</span></div>
            </div>
        </div>`;
    lucide.createIcons();
    attachListeners();
...
const renderLayerInput = (label, key, data, options, minVal, isSurface = false) => `
    <div class="card p-4 layer-card" data-layer="${key}">
        <div class="flex justify-between items-center mb-3">
            <h4 class="font-semibold text-slate-700 flex items-center gap-2"><i data-lucide="${isSurface ? 'layers' : 'box'}" class="w-4 h-4 text-dnit-500"></i> ${label}</h4>
            <span class="text-xs bg-slate-100 px-2 py-1 rounded text-slate-600">K = <span id="k-${key}">-</span></span>
        </div>
        <div class="grid grid-cols-3 gap-3">
            <div class="col-span-2"><label class="label-text">Material</label><select id="mat-${key}" class="input-field text-sm py-1">${options}</select></div>
            <div><label class="label-text">Espessura (cm)</label><input type="number" id="thick-${key}" value="${data.thickness}" class="input-field text-sm py-1" min="0" step="1"></div>
        </div>
        ${!isSurface ? `<div class="mt-2"><label class="label-text">CBR do Material (%)</label><input type="number" id="cbr-${key}" value="${data.cbr || 0}" class="input-field text-sm py-1 w-1/3"></div>` : ''}
...
function attachListeners() {
    ...
    layers.forEach(layer => {
        document.getElementById(`mat-${layer}`).addEventListener('change', (e) => { store.updateNested(`structure.${layer}.materialId`, e.target.value); recalculate(); });
        document.getElementById(`thick-${layer}`).addEventListener('input', (e) => { store.updateNested(`structure.${layer}.thickness`, Number(e.target.value)); recalculate(); });
        if (layer !== 'surface') { document.getElementById(`cbr-${layer}`).addEventListener('input', (e) => { store.updateNested(`structure.${layer}.cbr`, Number(e.target.value)); recalculate(); }); }
    });
}
...
function recalculate() {
    ...
    const H20 = calculateTotalThickness(N, 20);
    const Hn = calculateTotalThickness(N, s.reinforcement.cbr > 0 ? s.reinforcement.cbr : state.soil.subgradeCBR);
    const Hm = calculateTotalThickness(N, state.soil.subgradeCBR);
    ...
    if (s.reinforcement.thickness > 0) { const H_reinf_req = calculateTotalThickness(N, s.reinforcement.cbr); if (Eq_Total_Subbase < H_reinf_req) msgs.push(`⚠️ Estrutura insuficiente sobre o Reforço (Req: ${H_reinf_req.toFixed(1)} cm eq).`); }
    ...
    if (msgs.length > 0) { valBox.innerHTML = msgs.map(m => `<p>${m}</p>`).join(''); valBox.classList.remove('hidden'); }
    else { valBox.innerHTML = `<p class="text-emerald-600 font-semibold flex items-center gap-2"><i data-lucide="check-circle" class="w-4 h-4"></i> Dimensionamento atende aos critérios do DNIT.</p>`; valBox.classList.remove('hidden'); lucide.createIcons(); }
    drawPavement(s);
}
...
function drawPavement(layers) {
    ...
    // We want to show the pavement structure + a chunk of subgrade
    const pavementThick = layers.surface.thickness + layers.base.thickness + layers.subbase.thickness + layers.reinforcement.thickness;
    const minSubgrade = 30; // Minimum subgrade to show in cm
    const totalDisplayThick = Math.max(pavementThick + minSubgrade, 60); // Minimum 60cm total view

    const scale = (h - 40) / totalDisplayThick; // Leave 20px padding top/bottom
    let currentY = 20;

    const drawLayer = (thickness, color, label, pattern = null) => {
        ...
        // Label (Right)
        ctx.textAlign = 'left';
        ctx.fillText(label, w - 45, currentY + hPx / 2);

        // Thickness (Left)
        ctx.textAlign = 'right';
        ctx.fillText(`${thickness} cm`, 45, currentY + hPx / 2);
    ...
    // Draw Subgrade (fills the rest)
    const subgradeY = currentY;
    const subgradeH = h - subgradeY - 20; // Leave 20px bottom padding

    if (subgradeH > 0) {
        ctx.fillStyle = '#885533';
        ctx.fillRect(50, subgradeY, w - 100, subgradeH);

        ctx.strokeStyle = 'rgba(0,0,0,0.2)';
        ctx.strokeRect(50, subgradeY, w - 100, subgradeH);
        ...
        ctx.fillText('Subleito', w / 2, subgradeY + 10);

        // Infinite symbol or arrow to indicate it goes down
        ctx.beginPath();
        ctx.moveTo(w / 2, subgradeY + 30);
        ctx.lineTo(w / 2, subgradeY + 45);
        ctx.lineTo(w / 2 - 5, subgradeY + 40);
        ctx.moveTo(w / 2, subgradeY + 45);
        ctx.lineTo(w / 2 + 5, subgradeY + 40);
