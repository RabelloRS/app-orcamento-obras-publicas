...
export const calculateKirpich = (L, H) => {
    ...
    return Math.max(5, tc); // Minimum of 5-10 min usually recommended for urban areas
};
...
export const calculateEnvelope = (areaHa, C, idf, tr, limitQ_m3s, availableVol_m3, mode) => {
    ...
    if (mode === 'volume') {
        ...
        let low = 0;
        let high = calculateRationalQ(C, calculateIntensity(idf, tr, 5), areaHa) * 1.5; // Upper bound estimate
        let bestQ = 0;
        const tolerance = 0.001; // m3/s
        ...
        for (let k = 0; k < 20; k++) {
            ...
            if (currentMaxVol > availableVol_m3) {
                ...
                bestQ = midQ;
                high = midQ; // If currentMaxVol < Available, we have slack, so we could have released LESS? 
        ...
        calculatedLimitQ = bestQ; // This is the Minimum Outflow Capacity required to utilize the volume fully.
